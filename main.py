...
def get_creation_time(file_path):
    ...
    if platform.system() == 'Windows':
        ...
        try:
            ...
            # Em sistemas Linux, onde `st_birthtime` não está disponível,
            # vamos usar a última modificação como fallback.
...
def organize_videos_by_date(folder_path):
    ...
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            # Aqui você pode adicionar mais extensões de arquivo conforme necessário.
            if file.lower().endswith(('.mp4', '.avi', '.mov', '.mkv')):
                file_path = os.path.join(root, file)
                creation_time = get_creation_time(file_path)
                date_folder = datetime.fromtimestamp(creation_time).strftime('%Y-%m-%d')
                
                new_folder_path = os.path.join(root, date_folder)
                
                if not os.path.exists(new_folder_path):
                    os.makedirs(new_folder_path)
                
                shutil.move(file_path, new_folder_path)
                print(f'Movido: {file_path} -> {new_folder_path}')
